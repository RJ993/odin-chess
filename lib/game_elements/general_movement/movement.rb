...
module Movement
  def move(player, opposing_player, board, input)
    new_square = castling?(input, board) if ['o-o', 'o-o-o'].include?(input) && self.class == King
    return new_square if new_square == 'CASTLED'

    moved == true if [King, Rook].include?(self.class)
  ...
  def takes(_taking_player, opposing_player, current_square, new_square, taken_piece = [])
