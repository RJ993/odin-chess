...
module En_Passant
  def en_passant(_taking_player, opposing_player, passed_square, taken_piece)
  ...
  def en_passant?(board, loc)
    ...
    if color == 'white'
      if board.layout[loc].designation[1] == '5'
        if board.layout[loc + 1].piece.class == Pawn && board.layout[loc + 1].piece.color == 'black' && board.layout[loc + 1].piece.en_passant_able == true
          array += [[1,
                     1]]
        end
        if board.layout[loc - 1].piece.class == Pawn && board.layout[loc - 1].piece.color == 'black' && board.layout[loc - 1].piece.en_passant_able == true
    ...
    elsif board.layout[loc].designation[1] == '4'
      if board.layout[loc + 1].piece.class == Pawn && board.layout[loc + 1].piece.color == 'white' && board.layout[loc + 1].piece.en_passant_able == true
        array += [[1,
                   -1]]
      end
      if board.layout[loc - 1].piece.class == Pawn && board.layout[loc - 1].piece.color == 'white' && board.layout[loc - 1].piece.en_passant_able == true
