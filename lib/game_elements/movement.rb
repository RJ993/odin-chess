...
module Movement
  ...
  def prep_move_pos(board)
      ...
      calculate_pos(board, new_cords, cords)
  end

  def calculate_pos(board, new_cords, cords)
  ...
  def move(player, game, board, input)
    ...
    new_square = board.layout.find{|square| square.designation == input} if self.move_pos.include?(input)
      if new_square != nil && new_square.piece == nil
        ...
        self.pawn_conditions(player) if self.class == Pawn
